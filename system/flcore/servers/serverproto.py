...
class FedProto(Server):
    ...
    #     # 载入历史全局原型
    #     hist_global_protos = load_item(self.role, 'global_protos', self.save_folder_name)

    #     # 更新全局原型，更新的全局原型被替换，没有更新的使用历史的
    #     if hist_global_protos is not None:
    #         # 合并新旧全局原型字典，新的覆盖旧的
    #         merged_protos = dict(hist_global_protos)  # 复制历史原型
    #         merged_protos.update(global_protos)        # 用新原型更新/替换
    #         global_protos = merged_protos
