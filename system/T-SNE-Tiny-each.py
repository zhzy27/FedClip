    ...
    try:
        ...
        with torch.no_grad():
            for batch_idx, (images, labels) in enumerate(client_dataloader):
                # # 限制每个客户端的数据量，避免内存不足
                # if batch_idx >= 50:  # 调整为适合TinyImageNet的值
                #     break
                features = model.base(images)
                client_features.append(features.cpu().numpy())
                client_labels.append(labels.cpu().numpy())
                
        if len(client_features) == 0:
            print(f"警告: 客户端 {client_id} 没有数据")
            return None
...
if __name__ == "__main__":
    # 配置参数
    model_path_dir="./temp/TinyImagenet/FedDAR/0.1"
    excel_result_dir="./T-SNE/FedDAR/TINY/Hetero/test"
