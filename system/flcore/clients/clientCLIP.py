...
class clientCLIP(Client):
    ...
    def estimate_local_train_flops(self):
        ...
        record["model_forward_local_train_flops"] = float(
            record.get("local_train_flops", 0.0)
        )
        ...
        regular_forward_flops_per_batch, regular_module_flops = (
            estimate_low_rank_frobenius_flops(model)
        )
        ...
        regular_forward_flops_per_epoch = (
            regular_forward_flops_per_batch * batches_per_epoch
        )
        regular_local_train_flops = (
            regular_forward_flops_per_epoch * local_epochs * train_multiplier
        )

        record["regular_forward_flops_per_batch"] = float(
            regular_forward_flops_per_batch
        )
        record["regular_forward_flops_per_epoch"] = float(
            regular_forward_flops_per_epoch
        )
        record["regular_local_train_flops"] = float(regular_local_train_flops)
        record["regular_module_flops"] = regular_module_flops
        record["local_train_flops"] = (
            float(record.get("local_train_flops", 0.0))
            + float(regular_local_train_flops)
        )
        record["note"] = (
            f"{record.get('note', '')}+fedclip_frobenius_decay_x_train_multiplier"
        )
        return record
