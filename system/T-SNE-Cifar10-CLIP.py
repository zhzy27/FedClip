...
clip_text_features,clip_text_features_norm = get_clip_class_embeddings('Cifar10',model_name= "ViT-B/32",prompt_template= "{}")
        ...
        try:
            ...
            with torch.no_grad():
                for batch_idx, (images, labels) in enumerate(client_dataloader):
                    if batch_idx >= 20:  # 限制每个客户端的数据量，避免内存不足
                        break
...
if __name__ == "__main__":
    ...
    print("开始统一t-SNE降维...")
    unified_results = unified_tsne_across_clients(model_path_dr="./temp/Cifar10/FedCLIP/1775986079.8262901",
                                num_clients=20,
                                excel_result_dir="./T-SNE/FedCLIP/Cifar10/cos_align")
