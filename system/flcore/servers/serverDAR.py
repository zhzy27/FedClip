...
import random
import time
from flcore.clients.clientDAR import clientDAR
...
class FedDAR(Server):
    def __init__(self, args, times):
        ...
        if self.args.is_TT_Decom:
            global_model.TT_recover_larger_model()
        else:
    ...
    #客户端base进行参数对齐并且进行对齐后聚合
    def aggregate_parameters_TT(self):
        assert (len(self.uploaded_ids) > 0)
        #载入全局模型,全局模型是完整模型状态
        global_model = load_item(self.role, 'model', self.save_folder_name).to(self.device)
        for param in global_model.parameters():
